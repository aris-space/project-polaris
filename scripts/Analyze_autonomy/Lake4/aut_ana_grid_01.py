...
def draw_current_arrows(ax):
    # Row of single-headed arrows pointing up (+Y), each spanning Y = -24 -> -22,
    # placed at every X from -22 to 6 in 2 m steps.
    for x in np.arange(-22, 6 + 0.001, 2):
        ax.add_patch(FancyArrowPatch((x, -24), (x, -22),
...
def plot_data(csv_file_path, start_time=None, end_time=None, foxglove_offset=0.0):
    ...
    ax4.set_title(r'\textbf{$X$--$Y$ plane view of 20\,\textrm{m} by 20\,\textrm{m} Grid --- AUV path vs.\ reference path}')
    ax4.set_aspect('equal', adjustable='box')
    ax4.set_xlim(-24, 7)
    ax4.set_ylim(bottom=-24.5)  # keep the current-arrow row in view
