...
def plot_data(csv_file_path):
    ...
    # Axes swapped: plot Y on horizontal, X on vertical
    ax4.plot(path_xy['closest_y_m'], path_xy['closest_x_m'],
             '--', color='black', linewidth=1.5, label='Actual reference path')
    # Initial reference path: straight line from (x,y)=(0,0) to (0,10)
    ax4.plot([0, 10], [0, 0], '--', color='#1f77b4', linewidth=1.5,
             label='Initial reference path')

    # Robot trajectory colored by elapsed time
    rx = df['robot_x_m'].to_numpy()
    ry = df['robot_y_m'].to_numpy()
    t = time.to_numpy()
    points = np.array([ry, rx]).T.reshape(-1, 1, 2)
    ...
    # Start / end markers
    ax4.plot(ry[0], rx[0], 'o', color='green', markersize=10,
             markeredgecolor='black', label='Start', zorder=5)
    ax4.plot(ry[-1], rx[-1], 's', color='red', markersize=10,
             markeredgecolor='black', label='End', zorder=5)

    ax4.set_xlabel('North / Y [m]')
    ax4.set_ylabel('East / X [m]')
    ax4.set_title('X-Y Plane View (ROS ENU) — Robot Trajectory vs Reference Path')
    ax4.set_aspect('equal', adjustable='box')

    # Tight limits based on data extent (robot + path), with a small margin
    all_x = np.concatenate([rx, path_xy['closest_x_m'].to_numpy()])
    all_y = np.concatenate([ry, path_xy['closest_y_m'].to_numpy()])
    margin = 0.5
    ax4.set_xlim(all_y.min() - margin, all_y.max() + margin)
    ax4.set_ylim(all_x.min() - margin, all_x.max() + margin)
    ...
    # Subsample to ~750 frames (30 fps × 25 s) for smoother playback
    n_frames = 750
    n = len(t)
    ...
    # Static background: reference paths, start, end (axes swapped: Y horizontal, X vertical)
    ax5.plot(path_xy['closest_y_m'], path_xy['closest_x_m'],
             '--', color='black', linewidth=1.5, label='Actual reference path')
    # Initial reference path: straight line from (x,y)=(0,0) to (0,10)
    ax5.plot([0, 10], [0, 0], '--', color='#1f77b4', linewidth=1.5,
             label='Initial reference path')
    ax5.plot(ry[0], rx[0], 'o', color='green', markersize=10,
             markeredgecolor='black', label='Start', zorder=5)
    ax5.plot(ry[-1], rx[-1], 's', color='red', markersize=10,
             markeredgecolor='black', label='End', zorder=5)

    ax5.set_xlabel('North / Y [m]')
    ax5.set_ylabel('East / X [m]')
    ax5.set_title('X-Y Plane Animation (ROS ENU) — Robot + Yaw')
    ax5.set_aspect('equal', adjustable='box')
    all_x_anim = np.concatenate([rx, path_xy['closest_x_m'].to_numpy()])
    all_y_anim = np.concatenate([ry, path_xy['closest_y_m'].to_numpy()])
    margin = 0.5
    ax5.set_xlim(all_y_anim.min() - margin, all_y_anim.max() + margin)
    ax5.set_ylim(all_x_anim.min() - margin, all_x_anim.max() + margin)
    ...
    def update(i):
        # Trail up to current frame (plot Y horizontal, X vertical)
        cur_end = idx[i] + 1
        pts = np.array([ry[:cur_end], rx[:cur_end]]).T.reshape(-1, 1, 2)
        if len(pts) >= 2:
            segs = np.concatenate([pts[:-1], pts[1:]], axis=1)
            trail.set_segments(segs)
            trail.set_array(t[:cur_end - 1])
        # Robot + yaw stick — psi is heading in (X=East, Y=North); on swapped axes
        # the horizontal component is sin(psi) (Y direction) and vertical is cos(psi) (X direction)
        x, y, psi = rx_a[i], ry_a[i], yaw_a[i]
        robot_dot.set_data([y], [x])
        yaw_stick.set_data([y, y + stick_len * np.sin(psi)],
                           [x, x + stick_len * np.cos(psi)])
        ...
        lookahead_arrow.set_positions((y, x), (ly, lx))
        # Time-bar cover: shrink from the right as time advances
        cur_t = t_a[i]
        cover.set_bounds(cur_t, 0, t.max() - cur_t, 1)
...
if __name__ == "__main__":
    #csv_path = "C:\\Users\\pzzsc\\Documents\\FP_official_Programming\\Other things\\Autonomy_analyzation\\Autonomy_analyzation\\orca_tracking_csv_export_bad_yaw_turn\\tracking_errors_wide.csv"
