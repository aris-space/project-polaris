...
class MavlinkBridgeSender(Node):
    ...
    def __init__(self):
        ...
        self.battery_diagnostics_publisher = self.create_publisher(
    ...
    def handle_battery(self, msg):
        ...
        self.publish_battery_diagnostics(msg, ros_msg)
        self.logger.info(
            f"Published Battery: Current={ros_msg.current:.2f}A, Remaining={ros_msg.percentage:.0%}"
        )

    def publish_battery_diagnostics(self, mavlink_msg, battery_msg):
        """Publish battery state as a DiagnosticArray status."""
        diag_array = DiagnosticArray()
        diag_array.header.stamp = self.get_clock().now().to_msg()

        diag_status = DiagnosticStatus()
        diag_status.name = "pixhawk/battery"
        diag_status.hardware_id = f"sys={self.port.target_system},comp={getattr(self.port, 'target_component', 'unknown')}"

        if battery_msg.percentage < 0.0:
            diag_status.level = DiagnosticStatus.WARN
            diag_status.message = "Battery remaining unavailable"
        elif battery_msg.percentage <= 0.15:
            diag_status.level = DiagnosticStatus.ERROR
            diag_status.message = "Battery critical"
        elif battery_msg.percentage <= 0.30:
            diag_status.level = DiagnosticStatus.WARN
            diag_status.message = "Battery low"
        else:
            diag_status.level = DiagnosticStatus.OK
            diag_status.message = "Battery nominal"

        valid_voltages_mv = [
            voltage for voltage in mavlink_msg.voltages if 0 < voltage < 65535
        ]
        total_voltage = (
            sum(valid_voltages_mv) / 1000.0 if valid_voltages_mv else float("nan")
        )

        diag_status.values = [
            KeyValue(key="battery_id", value=str(mavlink_msg.id)),
            KeyValue(key="current_a", value=f"{battery_msg.current:.3f}"),
            KeyValue(
                key="remaining_percent", value=f"{battery_msg.percentage * 100.0:.1f}"
            ),
            KeyValue(key="voltage_v", value=f"{total_voltage:.3f}"),
            KeyValue(key="temperature_c", value=str(mavlink_msg.temperature)),
        ]

        diag_array.status = [diag_status]
        self.battery_diagnostics_publisher.publish(diag_array)
